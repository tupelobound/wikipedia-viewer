...
$('#search').keydown(function (event) {
  if (event.keyCode === 13) { // 13 is key code for the enter key
    ...
    const searchTerm = $('#search').val() // new variable from user input
    ...
    $.getJSON(
            ...
            function (json) {
              ...
              let id = 1 // create an id to supply to ids of the result card divs as they are created

              for (let i = 0; i < json.query.pages.length; i++) { // for each of the JSON objects isolate:
                const title = json.query.pages[i].title // the title of the page
                const url = json.query.pages[i].canonicalurl // the url of the page
                const extract = json.query.pages[i].extract // the text overview of the page
